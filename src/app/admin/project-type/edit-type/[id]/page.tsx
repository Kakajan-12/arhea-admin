...
const EditType = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/project-type/${id}`, {
                    headers: {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('auth_token');
            await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/project-type/${id}`, data, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            router.push('/admin/project-type');
        } catch (err) {
