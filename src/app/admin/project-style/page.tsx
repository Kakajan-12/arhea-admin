...
const ProjectStyle = () => {
    const [style, setStyle] = useState<DataItem[]>([]);
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get(
                `${process.env.NEXT_PUBLIC_API_URL}/api/project-style`,
                {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await axios.delete(
                `${process.env.NEXT_PUBLIC_API_URL}/api/project-style/${selectedId}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );

            setStyle(prev => prev.filter(item => item.id !== selectedId));
        } catch (err) {
            console.error(err);
            ...
            <div className="flex-1 p-10 ml-62">
                ...
                <div className="mt-8">
                    <div className="w-full flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold">Project styles</h2>

                        <Link
                            href="/admin/project-style/add-style"
                            ...
                            style.map(item => (
                                <tr key={item.id} className="border-t">
                                    <td className="py-4 px-4">
                                        <div dangerouslySetInnerHTML={{ __html: item.style_tk }} />
                                    </td>
                                    <td className="py-4 px-4">
                                        <div dangerouslySetInnerHTML={{ __html: item.style_en }} />
                                    </td>
                                    <td className="py-4 px-4">
                                        <div dangerouslySetInnerHTML={{ __html: item.style_ru }} />
                                    </td>

                                    <td className="py-4 px-4">
                                        <Link
                                            href={`/admin/project-style/edit-style/${item.id}`}
